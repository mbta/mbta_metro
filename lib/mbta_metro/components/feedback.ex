defmodule MbtaMetro.Components.Feedback do
  ...
  def feedback(%{kind: :info} = assigns) do
    ~H"""
    <p class="mt-1 flex gap-2 text-sm leading-6 text-blue-600">
  ...
  def feedback(%{kind: :success} = assigns) do
    ~H"""
    <p class="mt-1 flex gap-2 text-sm leading-6 text-green-600">
  ...
  def feedback(%{kind: :warning} = assigns) do
    ~H"""
    <p class="mt-1 flex gap-2 text-sm leading-6 text-yellow-600">
